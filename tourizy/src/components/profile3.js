import React, { Component } from 'react';
import './profile.css'

import { Row, Col } from 'antd';
import mech1 from './mech1.jpg'

class Profile3 extends Component {
    render() {
        return (
            <div>

                <div class="container">
                    <img src="" alt="picture of a city" class="top-image" />
                    <div class="profile-pic"></div>
                    <div class="profile">
                        <div class="profile-info">
                            <div class="name">Name Here</div>
                        </div>
                        <div class="profile-popularity">
                            <div class="field"><a href="">Edit Profile</a></div>
                        </div>
                    </div>
                    <br /><br /><br /><br /><br /><br /><br /><br />
                    <br /><br /><br /><br /><br /><br />
                    <div className="card">
                        <div className="card-content">
                            <span className="card-title"><b>Account Details</b></span>
                            <table>
                                <tr>
                                    <td>Username:</td>
                                    <td>abcdef</td>
                                </tr>
                                <tr>
                                    <td>Country:</td><td>India</td>
                                </tr>
                                <tr>
                                    <td>State:</td><td>Haryana</td>
                                </tr>
                                <tr>
                                    <td>City:</td><td>Kurukshetra</td>
                                </tr>
                                <tr>
                                    <td>Showroom Address:</td><td>adhgdkkfdkv</td>
                                </tr>
                                <tr>
                                    <td>Mobile No.:</td><td>0123456789</td>
                                </tr>
                                <tr>
                                    <td>Email ID:</td><td>abc@gmail.com</td>
                                </tr>
                                <tr>
                                    <td>You Deal In:</td><td>Cars, Bikes</td>
                                </tr>
                                <tr>
                                    <td>Showroom Timings:</td><td>8:00 am To 8:00 pm</td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    <div className="card">
                        <div className="card-content">
                            <span className="card-title"><b>Your Top Deals Available</b></span>
                            <p>
                                <b>Name: Wagonr</b><br />
                                Model:asd<br/>
                                Features: <br />
                                Price: 12345<br/>
                            </p><br/>
                            <p>
                                <b>Name: Bucati</b><br />
                                Model:asd<br/>
                                Features: <br />
                                Price: 12345<br/>
                            </p><br/>
                            <p>
                                <b>Name: Range Rover</b><br />
                                Model:asd<br/>
                                Features: <br />
                                Price: 12345<br/>
                            </p>
                        </div>
                    </div>

                </div>
            </div>
        );
    }
}
export default Profile3;
